...
function fullTestFunction() {
  ...
  try {
    const startFromWhatYear=30;
    const removed = store.removeOutdatedBooks(startFromWhatYear);
    console.log("Removed", removed.length, "book(s) from ",startFromWhatYear, { outdatedBooks: removed });

improve the message ij log an the variable naem
