import { Book, DemoBook, EBook, PaperBook } from "../models/Book";
import { MailService } from "../services/MailService";
import { ShippingService } from "../services/ShippingService";

export class Bookstore {
  private books: Book[] = [];

  addBook(book: Book) {
    this.books.push(book);
    console.log("Book added ->", book);
  }

  removeOutdatedBooks(maxAge: number): Book[] {
    // const bookYear= 
    const currentYear = new Date().getFullYear();
    const outdated = this.books.filter(b => currentYear - b.publishedAt.getFullYear() > maxAge);
    this.books = this.books.filter(b => currentYear - b.publishedAt.getFullYear() <= maxAge);
    return outdated;
  }

  buyBook(isbn: string, quantity: number, email: string, address: string): number {
  const book = this.books.find(b => b.isbn === isbn);

  if (!book) {
    throw new Error(`Quantum book store: Book with ISBN ${isbn} not found.`);
  }

  if (book instanceof DemoBook) {
    throw new Error(`Quantum book store: '${book.title}' is a demo book and cannot be purchased.`);
  }

  if (book instanceof PaperBook) {
    if (book.stock < quantity) {
      throw new Error(`Quantum book store: Only ${book.stock} copy(ies) left of '${book.title}', cannot fulfill request of ${quantity}.`);
    }

    book.stock -= quantity;
    ShippingService(address, book, quantity);
    return quantity * book.price;
  }

  if (book instanceof EBook) {
    MailService(email, book);
    return quantity * book.price;
  }

  throw new Error("Quantum book store: Unsupported book type.");
}

}