...
class ShippingService {
  static ship(address: string, book: PaperBook, quantity: number) {
...
class Bookstore {
  ...
  addBook(book: Book) {
    this.bookds.push(book);
    console.log("Quantum book store: Book added ->", book);
  }

  removeOutdatedBooks(maxAge: number): Book[] {
    const currentYear = new Date().getFullYear();
    const outdated = this.bookds.filter(b => currentYear - b.year > maxAge);
    this.bookds = this.bookds.filter(b => currentYear - b.year <= maxAge);
  ...
  buyBook(isbn: string, quantity: number, email: string, address: string): number {
    const book = this.bookds.find(b => b.isbn === isbn);
...
class QuantumBookstoreFullTest {
  static run() {
    ...
    const books: Book[] = [
      ...
      new ShowcaseBook("SB001", "The Art of War", "Sun Tzu", -500),
      new ShowcaseBook("SB002", "Principia Mathematica", "Isaac Newton", 1687),
      new ShowcaseBook("SB003", "On the Origin of Species", "Charles Darwin", 1859),
