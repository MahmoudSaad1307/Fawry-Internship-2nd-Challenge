...
class QuantumBookstore {
  ...
  removeOutdatedBooks(maxAge: number): Book[] {
    ...
    const outdated = this.inventory.filter(b => currentYear - b.year > maxAge);
    this.inventory = this.inventory.filter(b => currentYear - b.year <= maxAge);
...
class QuantumBookstoreFullTest {
  static run() {
    const store = new QuantumBookstore();
